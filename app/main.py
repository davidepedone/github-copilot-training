...
@app.post("/log_task")
async def log_task(task: DeveloperTask) -> Dict[str, str | int]:
    """
    Add a new task to the productivity tracking system.
    
    **Endpoint:** POST /log_task
    
    **Parameters:**
    - `task` (DeveloperTask): A task object containing:
        - `task_id`: int (will be overwritten with auto-generated ID)
        - `title`: str (required) - Task description
        - `status`: TaskStatus (default: PENDING) - One of: pending, in_progress, complete
        - `hours_spent`: float (default: 0.0) - Hours spent on the task
    
    **Returns:** Dict with:
    - `message`: str - Confirmation message
    - `task_id`: int - The newly assigned task ID
    ...
    **Behavior:**
    1. Generates a new unique task ID (max existing ID + 1)
    2. Assigns the ID to the incoming task
    3. Stores the task in the mock database
    4. Returns confirmation with the assigned ID
    """
    new_id = max(MOCK_TASKS.keys()) + 1 if MOCK_TASKS else 1
    task.task_id = new_id
    MOCK_TASKS[new_id] = task
    
    return {"message": f"Task ID {task.task_id} logged successfully.", "task_id": task.task_id}
